...
module.exports = function () {
    ...
    this.Given(/^I receive the following requests:$/, function (table, callback) {
        ...
        var sendAllMessagesPromises = table.raw().map(function (row) {
            return world.requestQueue.sendTextMessage(row[0]);
        });
        world.requestCount = sendAllMessagesPromises.length;

        Promise.all(sendAllMessagesPromises)
            .then(proceed(callback), orReportException(callback));
    });
